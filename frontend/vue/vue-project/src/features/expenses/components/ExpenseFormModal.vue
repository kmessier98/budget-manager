...
const date = computed(() => {
  if (isEditMode.value) {
    ...
    const day = filters.value.day ? String(filters.value.day).padStart(2, "0") : "01";
    const month = filters.value.month ? String(filters.value.month).padStart(2, "0") : "01";
...
const categoryId = computed(() => {
  if (isEditMode.value) {
    ...
    let defaultCategoryId = categories.value.find((cat) => cat.value === filters.value.categoryId)?.value;
...
async function handleFetchExpenses() {
  try {
    ...
    toast.add({
      severity: "error",
      summary: "Erreur",
      detail: "Une erreur est survenue lors du chargement des dépenses.",
      life: 3000,
    });
  }
}
...
async function handleUpdateExpense(formValues: ExpenseFormValues) {
  try {
    ...
    toast.add({
      severity: "error",
      summary: "Erreur",
      detail: "Une erreur est survenue lors de la mise à jour de la dépense.",
      life: 3000,
    });
  }
}
...
<template>
  ...
  >
    ...
    >
      ...
      <span v-if="submitError || fetchError" class="error-message-global visible">{{ submitError || fetchError }}</span>
    </Form>
  </Dialog>
</template>
