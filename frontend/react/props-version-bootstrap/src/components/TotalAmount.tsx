...
const TotalAmount = ({
  ...
  const getFormatedText = () => {
    ...
    if (summaryExpense.day) {
      return `Total du ${date.getDate()} ${date.toLocaleString("fr-FR", { month: "long" })} ${date.getFullYear()} : ${summaryExpense.totalAmount} $`;
      ...
      if (
        ...
        return `Total du mois d'${date.toLocaleString("fr-FR", { month: "long" })} ${date.getFullYear()} : ${summaryExpense.totalAmount} $`;
      }
      return `Total du mois de ${date.toLocaleString("fr-FR", { month: "long" })} ${date.getFullYear()} : ${summaryExpense.totalAmount} $`;
    }

    return `Total de l'année ${date.getFullYear()} : ${summaryExpense.totalAmount} $`;
  };
