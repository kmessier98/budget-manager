import { type ExpenseFormValues, type ExpenseResponse } from "../models/expense/expenses";

export const expenseService = {
  fetchExpenses: async (queryParams: Record<string, string | number>): Promise<ExpenseResponse> => {
    const queryString = new URLSearchParams(queryParams as Record<string, string>).toString();
    const response = await fetch(`https://localhost:7208/api/transaction?${queryString}`);
    if (!response.ok) {
  ...
  updateExpense: async (formData: ExpenseFormValues) => {
    const response = await fetch(`https://localhost:7208/api/transaction/${formData.id}`, {
      method: "PUT",
      body: JSON.stringify(formData),
      headers: {
        "Content-Type": "application/json",
